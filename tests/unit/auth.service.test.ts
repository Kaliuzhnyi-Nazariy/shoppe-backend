...
describe("authService.resetPassword", () => {
  ...
  it("should update user password", async () => {
    ...
    (prisma.passwordResetTokens.findUnique as jest.Mock).mockResolvedValue({
      id: "token-1",
      userId: userData.id,
      token: "token",
    });
    ...
    (prisma.passwordResetTokens.delete as jest.Mock).mockResolvedValue({
    ...
    expect(prisma.passwordResetTokens.findUnique).toHaveBeenCalledWith({
      where: { userId: tokenPayload.payload.id },
    });
    ...
    expect(prisma.passwordResetTokens.delete).toHaveBeenCalledWith({
      where: { userId: tokenPayload.payload.id },
    });
  });
